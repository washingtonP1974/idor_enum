...
func main() {
        if len(os.Args) != 3 {
                fmt.Println("Uso: go run main.go http://ip target host:port")
                os.Exit(1)
        }
        ...
        for i := 1; i <= maxID; i++ {
                req, err := http.NewRequest("GET", baseURL+targetHost+"/"+strconv.Itoa(i), nil)
                if err != nil {
                ...
                req, err = http.NewRequest("POST", baseURL+targetHost+"/"+strconv.Itoa(i), bytes.NewBuffer(jsonData))
                if err != nil {
